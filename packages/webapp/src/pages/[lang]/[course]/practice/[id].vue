...
const { getById, getPrev, getNext, phoneticsOf, speechTextOf } = useCourseData()
...
const phonetic = computed(() => phoneticsOf(charId.value, locale.value))
...
// Pre-fetch audio for the current character + its neighbors so the
// speaker button plays instantly on click and prev/next navigation feels
// snappy. The proxy responds with a 1-year `Cache-Control: immutable`,
// so these warmed bytes stay in browser cache for the whole session.
watch(char, (c) => {
  if (c) preload(speechTextOf(c.id, c.symbol))
}, { immediate: true })
watch(prev, (p) => {
  if (p) preload(speechTextOf(p.id, p.symbol))
}, { immediate: true })
watch(next, (n) => {
  if (n) preload(speechTextOf(n.id, n.symbol))
}, { immediate: true })
</script>
    ...
    <div v-if="drawMode && char" class="draw-overlay">
      <div class="draw-header">
        ...
        <button
          v-if="isSupported"
          class="draw-sound"
          :class="{ 'draw-sound--active': isSpeaking, 'draw-sound--loading': isLoading || isSpeaking }"
    ...
    <template v-if="!drawMode">
      <div class="page-container p__inner">
        ...
        <template v-if="char">
          ...
          <div class="quick-nav">
            <NuxtLink :to="paths.table" class="quick-nav__back">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
              {{ t(tKey('backToGrid')) }}
            </NuxtLink>
            <div class="quick-nav__arrows">
              <NuxtLink v-if="prev" :to="paths.practice(prev.id)" class="quick-nav__arrow" :aria-label="t(tKey('prevChar'))">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
                <span class="quick-nav__arrow-char">{{ prev.symbol }}</span>
              </NuxtLink>
              <span v-else class="quick-nav__placeholder" />
              <NuxtLink v-if="next" :to="paths.practice(next.id)" class="quick-nav__arrow" :aria-label="t(tKey('nextChar'))">
          ...
          <div class="char-hero">
            ...
            <div class="char-hero__info">
              ...
              <div class="char-hero__meta">
                <span>{{ t(tKey(`charType_${char.type}`)) }}</span>
                <span class="meta-dot">·</span>
                <span>{{ t(tKey(`charSubtype_${char.subtype}`)) }}</span>
                <span class="meta-dot">·</span>
                <span>{{ t(tKey('strokeCount'), { n: char.strokeCount }) }}</span>
              </div>
            </div>
            ...
            <div class="char-hero__actions">
              <button
                v-if="isSupported"
                class="act"
                :class="{ 'act--active': isSpeaking, 'act--loading': isLoading || isSpeaking }"
                :disabled="isLoading || isSpeaking"
              ...
              <button class="act act--primary" @click="drawMode = true" :aria-label="t(tKey('practice.write'))">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
                <span>{{ t(tKey('practice.write')) }}</span>
              </button>
            </div>
          </div>
          ...
          <section v-if="phonetic" class="card">
            <h2 class="card__title">{{ t(tKey('practice.phonetics')) }}</h2>
            <div class="rows">
              <div class="row">
              ...
              <div class="row">
                <span class="row__label">{{ t(tKey('practice.soundsLike')) }}</span>
                <span class="row__val">{{ phonetic.soundsLike }}</span>
              </div>
              <div class="row">
                <span class="row__label">{{ t(tKey('practice.mouthDesc')) }}</span>
          ...
          <nav class="char-nav">
            <NuxtLink v-if="prev" :to="paths.practice(prev.id)" class="nav-link">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg>
              <span class="nav-link__char">{{ prev.symbol }}</span>
              <span class="nav-link__label">{{ t(tKey('prevChar')) }}</span>
            </NuxtLink>
            <span v-else />
            <NuxtLink v-if="next" :to="paths.practice(next.id)" class="nav-link">
              <span class="nav-link__label">{{ t(tKey('nextChar')) }}</span>
        ...
        <template v-else>
          ...
          <NuxtLink :to="paths.table" class="btn btn--primary">{{ t(tKey('backToGrid')) }}</NuxtLink>
        </template>
      </div>
    </template>
  </div>
...
.char-hero {
  /* Force the card to always span the full inner width of `.p__inner`.
     Without this, the flex column parent lets the card size to its
     content (`max-content`), which makes the width visibly jump between
     a long character name (e.g. "Ssang-bieup") and a short one ("A"). */
  width: 100%;
...
.char-hero__display {
  /* Fixed-size square so the StrokeAnimation slot is identical for every
     character — no layout shift between mounts. */
...
.char-hero__info {
  /* Variable-length text (name, romanization, meta) wrapped in a
     fixed-width slot so a long name never widens its parent. The 320px
     ceiling fits the longest jamo names without forcing wrap on the
     short ones. */
  width: 100%;
  max-width: 320px;
...
.char-hero__name,
.char-hero__rom {
  /* Long names should ellipsis-truncate inside the 320px slot rather
     than push the slot wider. */
  max-width: 100%;
  overflow: hidden;
...
.act--loading { color: var(--color-primary); border-color: var(--color-primary); cursor: wait; }
.act--primary { background: var(--color-primary); color: #fff; border-color: var(--color-primary); }
.act--primary:hover { opacity: 0.85; }
.act:disabled { cursor: wait; opacity: 0.85; }

/* Spinning loader for the audio buttons (shared across .act / .draw-sound) */
...
.draw-sound--loading { color: var(--color-primary); cursor: wait; }
.draw-sound:disabled { cursor: wait; }
