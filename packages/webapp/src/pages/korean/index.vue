...
import heroImg from '~/assets/images/korea-hero.jpg'
definePageMeta({ layout: 'default', middleware: 'auth' })
const { t } = useI18n()
const { getBySlug } = useCourses()
const course = getBySlug('korean')!
const { profile } = useLanguageProfile('korean')
</script>

<template>
  <div class="lang" v-if="profile" :style="{ '--cc': course.color, '--cc-s': course.colorSubtle }">
    ...
    <section class="why-section">
      <div class="contained">
        <div class="why-header">
          <h2>{{ t('langProfile.korean.whyLearnTitle') }}</h2>
          <p>{{ t('langProfile.korean.whyLearnDesc') }}</p>
        </div>
    ...
    <section class="courses-section">
      <div class="contained">
        <div class="courses-header">
          <h2>{{ t('langProfile.korean.coursesTitle') }}</h2>
          <p>{{ t('langProfile.korean.coursesDesc') }}</p>
        </div>

        <div class="courses-grid">
          <NuxtLink
            v-for="c in profile.courses" :key="c.slug"
            :to="c.route"
            class="course-card"
        ...
        <div class="coming-soon">
          ...
          <div>
            <h4>{{ t('langProfile.korean.comingSoonTitle') }}</h4>
            <p>{{ t('langProfile.korean.comingSoonDesc') }}</p>
          </div>
        </div>
      </div>
    </section>
