  ...
  <div class="lp" :style="{ '--cc': course.color, '--cc-s': course.colorSubtle }">
    ...
    <section class="hero">
      <div class="hero__inner">
        <div class="hero__text">
          <span class="hero__pill">{{ course.flag }} {{ t('courses.korean') }}</span>
          <h1>{{ t('korean.hangeul') }}</h1>
          <p>{{ t('korean.hangeulDesc') }}</p>
          <div class="hero__actions">
            <NuxtLink to="/korean/hangeul/table" class="hero__ghost">{{ t('korean.hangeulSection.tableTitle') }}</NuxtLink>
            <NuxtLink to="/korean/hangeul/training" class="hero__cta">{{ t('korean.training') }}</NuxtLink>
          </div>
        </div>
    ...
    <section class="story" id="story">
      <div class="story__inner contained">
        <div class="story__lead">
          ...
          <h2>{{ t('korean.hangeulSection.introP1').split('.')[0] }}.</h2>
        </div>
        <div class="story__cols">
          <div class="story__col">
            <p>{{ t('korean.hangeulSection.introP1').split('. ').slice(1).join('. ') }}</p>
          </div>
          <div class="story__col">
            <p>{{ t('korean.hangeulSection.introP2') }}</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="quote-section">
      <div class="quote-section__inner contained">
        ...
        <blockquote>
          <p>{{ t('korean.hangeulSection.introP3') }}</p>
        </blockquote>
    ...
    <section class="band band--muted">
      <div class="contained">
        <span class="chip chip--cool">{{ t('korean.hangeulSection.jamoTitle') }}</span>
        <h2 class="band__title">{{ t('korean.hangeulSection.jamoConsonantTitle') }}</h2>
        <p class="band__sub">{{ t('korean.hangeulSection.jamoP1') }}</p>

        <div class="shape-row">
          <div class="shape-pill" v-for="item in t('korean.hangeulSection.jamoConsonantList').split(' | ')" :key="item">
    ...
    <section class="contained cosmo">
      <h2>{{ t('korean.hangeulSection.jamoCosmologyTitle') }}</h2>
      <p class="cosmo__sub">{{ t('korean.hangeulSection.jamoCosmologyP1') }}</p>

      <div class="cosmo__grid">
        <div class="cosmo__card cosmo__card--sky">
          <span class="cosmo__sym">·</span>
          <span class="cosmo__label">{{ t('korean.hangeulSection.jamoCosmologyHeaven') }}</span>
        </div>
        <div class="cosmo__card cosmo__card--earth">
          <span class="cosmo__sym">ㅡ</span>
          <span class="cosmo__label">{{ t('korean.hangeulSection.jamoCosmologyEarth') }}</span>
        </div>
        <div class="cosmo__card cosmo__card--human">
          <span class="cosmo__sym">ㅣ</span>
          <span class="cosmo__label">{{ t('korean.hangeulSection.jamoCosmologyHuman') }}</span>
        </div>
      </div>
    ...
    <section class="band band--warm">
      <div class="contained">
        <span class="chip chip--warm">{{ t('korean.hangeulSection.consonantsTitle') }}</span>
        <h2 class="band__title">{{ t('korean.basicConsonants') }} <span class="light">({{ basicConsonants.length }})</span></h2>
        <p class="band__sub">{{ t('korean.hangeulSection.consonantsIntro') }}</p>

        <div class="char-cards">
          <NuxtLink v-for="c in basicConsonants" :key="c.id" :to="`/korean/hangeul/practice/${c.id}`" class="cc">
        ...
        <h3 class="mt">{{ t('korean.doubleConsonants') }} <span class="light">({{ doubleConsonants.length }})</span></h3>
        <p class="band__sub">{{ t('korean.hangeulSection.doubleConsonantsIntro') }}</p>
        <div class="char-cards char-cards--sm">
          <NuxtLink v-for="c in doubleConsonants" :key="c.id" :to="`/korean/hangeul/practice/${c.id}`" class="cc">
            <span class="cc__sym" :style="{ color: course.color }">{{ c.symbol }}</span>
            <span class="cc__rom">{{ c.romanization }}</span>
            <span class="cc__name">{{ c.name }}</span>
          </NuxtLink>
    ...
    <section class="contained vowels-section">
      <span class="chip chip--cool">{{ t('korean.hangeulSection.vowelsTitle') }}</span>
      <h2>{{ t('korean.basicVowels') }} <span class="light">({{ basicVowels.length }})</span></h2>
      <p class="section-intro">{{ t('korean.hangeulSection.vowelsIntro') }}</p>

      <div class="char-cards">
        <NuxtLink v-for="c in basicVowels" :key="c.id" :to="`/korean/hangeul/practice/${c.id}`" class="cc">
      ...
      <h3 class="mt">{{ t('korean.compoundVowels') }} <span class="light">({{ compoundVowels.length }})</span></h3>
      <p class="section-intro">{{ t('korean.hangeulSection.compoundVowelsIntro') }}</p>
      <div class="char-cards char-cards--sm">
        <NuxtLink v-for="c in compoundVowels" :key="c.id" :to="`/korean/hangeul/practice/${c.id}`" class="cc">
          <span class="cc__sym" style="color: var(--color-primary)">{{ c.symbol }}</span>
          <span class="cc__rom">{{ c.romanization }}</span>
          <span class="cc__name">{{ c.name }}</span>
        </NuxtLink>
    ...
    <section class="band band--indigo">
      ...
      <p class="rule-demo__text">{{ t('korean.hangeulSection.silentIeungRule') }}</p>
    </section>

    <!-- ════════ SYLLABLES — contained, split visual/text (reversed) ════════ -->
    <section class="contained split split--rev">
      <div class="split__text">
        <span class="chip chip--green">{{ t('korean.hangeulSection.syllablesTitle') }}</span>
        <h2>{{ t('korean.hangeulSection.syllablesIntro').split(':')[0] }}</h2>
        <p>{{ t('korean.hangeulSection.syllablesBlockExplanation') }}</p>
      </div>
      <div class="split__diagrams">
        <div class="dg">
          <span class="dg__label">{{ t('korean.hangeulSection.cvPattern') }}</span>
          <SyllableBlockDiagram mode="cv" :example="{ initial: 'ㄱ', medial: 'ㅏ', result: '가', romanization: 'ga' }" />
        </div>
        <div class="dg">
          <span class="dg__label">{{ t('korean.hangeulSection.cvcPattern') }}</span>
    ...
    <section class="cta-section">
      <div class="contained cta-grid">
        ...
        <NuxtLink to="/korean/hangeul/training" class="cta cta--training">
          ...
          <div class="cta__body">
            <h3>{{ t('korean.training') }}</h3>
            <p>{{ t('korean.trainingDesc') }}</p>
          </div>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
        </NuxtLink>
      </div>
