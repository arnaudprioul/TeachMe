...
// Load SVG for guide overlay
const svgModules = import.meta.glob('~/assets/images/hangeul/*.svg', { query: '?raw', import: 'default', eager: true }) as Record<string, string>
...
<template>
  <div class="wc">
    ...
    <div class="wc__area">
      <div class="wc__stack">
        <!-- SVG guide layer (below canvas) -->
        <div
          v-if="showGuide && svgGuide"
          class="wc__guide"
          v-html="svgGuide"
        />
    ...
    <button class="wc__toggle" :class="{ 'wc__toggle--on': showGuide }" @click="showGuide = !showGuide">
      ...
      {{ t('korean.practice.guidedMode') }}
    </button>

    <!-- Guides visibility -->
    <div class="wc__guides">
      <span class="wc__guides-label">{{ t('korean.practice.guidesVisibility') }}</span>
